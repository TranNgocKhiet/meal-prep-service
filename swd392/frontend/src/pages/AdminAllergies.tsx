...
const AdminAllergies = () => {
  ...
  const [filtered, setFiltered] = useState<Allergy[]>([]);
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      const response = await apiClient.get('/admin/allergies');
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await apiClient.delete(`/admin/allergies/${id}`);
      fetchItems();
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        await apiClient.put(`/admin/allergies/${editingItem.id}`, formData);
      } else {
        await apiClient.post('/admin/allergies', formData);
      }
      setShowModal(false);
      fetchItems();
    } catch (err: any) {
    ...
    <div className="container">
      <div className="crud-header">
        ...
        <div className="crud-actions">
          ...
          />
          <button onClick={handleCreate} className="btn-primary">Add Allergy</button>
        </div>
      </div>
      ...
      {showModal && (
        <div className="modal-overlay">
          <div className="modal-content">
            <h2>{editingItem ? 'Edit Allergy' : 'Add Allergy'}</h2>
